...
static uint32_t pt[2048] __attribute__((aligned(16)));
...
// This function is different from the original Haribote
void set_palette(int start, int end, unsigned char *rgb)
{
	int i;
	uint32_t addr;
	...
	pt[pt_index++] =  0 + (end-start+1)*4; // 1bit request/response (0/1), 31bit value length in bytes
...
void putfonts8_asc(char *vram, int xsize, int x, int y, char c, char *s)
{
	...
	char *nihongo = (char *) _task_a_nihongo, *font;
	int k, t;
...
void putblock8_8(char *vram, int vxsize, int pxsize, int pysize, int px0, int py0, char *buf, int bxsize)
{
	...
	for (y = 0; y < pysize; y++) {
		for (x = 0; x < pxsize; x++) {
			vram[(py0 + y) * vxsize + (px0 + x)] = (int) buf[y * bxsize + x];
		}
	}
	return;
}
